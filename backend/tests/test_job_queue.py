...
async def test_job_devices_processed_strictly_one_at_a_time(client: AsyncClient, unique_email):
    ...
    # Devices are chained to run strictly one at a time (see jobs.py's
    # create_job) - each item's finished_at must be no later than the next
    # item's started_at, proving they didn't run concurrently.
...
async def test_job_continues_after_unexpected_exception_mid_chain(
    ...
    # didn't stop the chain from ever attempting the second one.
    assert all(i["status"] == "failed" for i in items)
